...
public partial class Admin_create_role : System.Web.UI.Page
{
    ...
    public void GetRoles()
    {
        try
        {
            List<UserRoles> comps = UserRoles.GetAllUserRoles(conHT).Where(x => x.Id == Convert.ToInt32(Request.QueryString["id"])).ToList();
            if (comps.Count > 0)
            {
                btnSave.Text = "Update";
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            if (Page.IsValid)
            {
                ...
                {
                    if (CreateUser.CheckAccess(conHT, pageName, "Edit", Request.Cookies["ht_aid"].Value))
                    {
                        ca.RoleName = txtName.Text;
                        ca.Id = Convert.ToInt32(Request.QueryString["id"]);
                        ca.UpdatedOn = TimeStamps.UTCTime();
                        ca.Status = "Active";
                        int result = UserRoles.UpdateUserRoles(conHT, ca);
                        if (result > 0)
                        {
                            GetAllRoles();
                            GetRoles();
                ...
                {
                    if (CreateUser.CheckAccess(conHT, pageName, "Add", Request.Cookies["ht_aid"].Value))
                    {
                        ca.RoleName = txtName.Text;
                        ca.UpdatedOn = TimeStamps.UTCTime();
                        ca.Status = "Active";
                        int result = UserRoles.InsertUserRoles(conHT, ca);
                        if (result > 0)
                        {
                            txtName.Text = "";
                            GetAllRoles();
    ...
    public void GetAllRoles()
    {
        ...
        {
            strUserRoless = "";
            List<UserRoles> cas = UserRoles.GetAllUserRoles(conHT).OrderByDescending(s => Convert.ToDateTime(s.UpdatedOn)).ToList();
            int i = 0;
...
<td>" + ca.UpdatedOn.ToString("dd-MMM-yyyy") + @"</td>  
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conHT = new SqlConnection(ConfigurationManager.ConnectionStrings["conHT"].ConnectionString);
            if (CreateUser.CheckAccess(conHT, "create-role.aspx", "Delete", HttpContext.Current.Request.Cookies["ht_aid"].Value))
            {
                UserRoles cat = new UserRoles();
                cat.Id = Convert.ToInt32(id);
                cat.UpdatedOn = TimeStamps.UTCTime();
                int exec = UserRoles.DeleteUserRoles(conHT, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
