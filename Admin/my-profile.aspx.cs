...
public partial class Admin_my_profile : System.Web.UI.Page
{
    ...
    public void GetProfileDetails()
    {
        try
        {
            List<CreateUser> comps = CreateUser.GetAllUser(conHT).Where(x => x.UserGuid == Request.Cookies["ht_aid"].Value).ToList();
            if (comps.Count > 0)
            {
        ...
        try
        {
            if (Page.IsValid)
            {
                ...
                if (CreateUser.CheckAccess(conHT, pageName, "Edit", Request.Cookies["ht_aid"].Value))
                {
                    ...
                    comp.UserGuid = Request.Cookies["ht_aid"].Value;
                    comp.AddedBy = Request.Cookies["ht_aid"].Value;
                    ...
                    comp.Status = "Active";
                    int result = CreateUser.UpdateUserByUser(conHT, comp);
                    if (result > 0)
                    {
                        GetProfileDetails();
