...
public partial class Admin_my_profile : System.Web.UI.Page
{
    ...
    public void GetUserDetails()
    {
        try
        {
            List<CreateUser> comps = CreateUser.GetAllUser(conHT).Where(x => x.Id ==Convert.ToInt32(Request.QueryString["id"])).ToList();
            if (comps.Count > 0)
            {
                btnSave.Text = "Update";
    ...
    public void BindRoles()
    {
        ...
        {
            ddlRole.Items.Clear();
            List<UserRoles> comps = UserRoles.GetAllUserRoles(conHT);
            if (comps.Count > 0)
            {
            ...
            if (Page.IsValid)
            {
                ...
                if (btnSave.Text == "Update")
                {
                    ...
                    if (CreateUser.CheckAccess(conHT, pageName, "Edit", Request.Cookies["ht_aid"].Value))
                    {
                        ...
                        ca.Status = "Active";
                        ca.AddedBy = Request.Cookies["ht_aid"].Value;
                        int result = CreateUser.UpdateUser(conHT, ca);
                        if (result > 0)
                        {
                            GetUserDetails();
                ...
                else
                {
                    if (CreateUser.CheckAccess(conHT, pageName, "Add", Request.Cookies["ht_aid"].Value))
                    {
                        ...
                        ca.Status = "Active";
                        ca.AddedBy = Request.Cookies["ht_aid"].Value;
                        int result = CreateUser.InsertUser(conHT, ca);
                        if (result > 0)
                        {
