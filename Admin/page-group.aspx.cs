...
public partial class Admin_page_group : System.Web.UI.Page
{
    ...
    public void GetGroups()
    {
        try
        {
            List<PageGroup> comps = PageGroup.GetAllPageGroup(conHT).Where(x => x.Id == Convert.ToInt32(Request.QueryString["id"])).ToList();
            if (comps.Count > 0)
            {
                btnSave.Text = "Update";
        ...
        try
        {
            if (Page.IsValid)
            {
                ...
                if (btnSave.Text == "Update")
                {
                    ...
                    ca.Status = "Active";
                    int result = PageGroup.UpdatePageGroup(conHT, ca);
                    if (result > 0)
                    {
                        GetAllGroup();
                        GetGroups();
                ...
                else
                {
                    ...
                    ca.UpdatedOn = TimeStamps.UTCTime();
                    ca.Status = "Active";
                    int result = PageGroup.InsertPageGroup(conHT, ca);
                    if (result > 0)
                    {
    ...
    public void GetAllGroup()
    {
        ...
        {
            strPageGroups = "";
            List<PageGroup> cas = PageGroup.GetAllPageGroup(conHT).OrderByDescending(s => Convert.ToDateTime(s.UpdatedOn)).ToList();
            int i = 0;
...
<td><a href='javascript:void();' class='bs-tooltip' data-id='" + ca.Id + @"' data-bs-toggle='tooltip' data-bs-placement='top' title=''>" + ca.UpdatedOn.ToString("dd-MMM-yyyy") + @"</a></td>  
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conHT = new SqlConnection(ConfigurationManager.ConnectionStrings["conHT"].ConnectionString);
            if (CreateUser.CheckAccess(conHT, "page-group.aspx", "Delete", HttpContext.Current.Request.Cookies["ht_aid"].Value))
            {
                PageGroup cat = new PageGroup();
                cat.Id = Convert.ToInt32(id);
                cat.UpdatedOn = TimeStamps.UTCTime();
                int exec = PageGroup.DeletePageGroup(conHT, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
