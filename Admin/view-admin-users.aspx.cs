...
public partial class Admin_view_admin_users : System.Web.UI.Page
{
    ...
    public void GetAllUsers()
    {
        ...
        {
            strUsers = "";
            List<CreateUser> instrs = CreateUser.GetAllUser(conHT);
            int i = 0;
...
<div class='form-check form-switch markBlocked' id='chk_" + cat.Id + @"'  data-id='" + cat.Id + @"'>
  ...
  <td>" + chk + @"</td> 
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conHT = new SqlConnection(ConfigurationManager.ConnectionStrings["conHT"].ConnectionString);
            if (CreateUser.CheckAccess(conHT, "view-admin-users.aspx", "Delete", HttpContext.Current.Request.Cookies["ht_aid"].Value))
            {
                ...
                cat.AddedBy = HttpContext.Current.Request.Cookies["ht_aid"].Value;
                int exec = CreateUser.DeleteUser(conHT, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
    ...
    public static string BlockUsers(string id, string ftr)
    {
        ...
        {
            SqlConnection conHT = new SqlConnection(ConfigurationManager.ConnectionStrings["conHT"].ConnectionString);
            if (CreateUser.CheckAccess(conHT, "view-admin-users.aspx", "Edit", HttpContext.Current.Request.Cookies["ht_aid"].Value))
            {
                int exec = 0;
                CreateUser pro = new CreateUser();
                if (ftr == "Yes")
                {
                    exec = CreateUser.BlockUser(conHT, id, HttpContext.Current.Request.Cookies["ht_aid"].Value);
                    vali = "Blocked";
                }
                else
                {
                    exec = CreateUser.UnBlockUser(conHT, id, HttpContext.Current.Request.Cookies["ht_aid"].Value);
                    vali = "UnBlocked";
                }
