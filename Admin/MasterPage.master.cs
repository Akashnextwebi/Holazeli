...
{
    SqlConnection conHT = new SqlConnection(ConfigurationManager.ConnectionStrings["conHT"].ToString());
    ...
    public void CheckPassKeyChanged()
    {
        try
        {
            if (Request.Cookies["ht_apkv"] != null)
            {
                string stored_pass_key = Request.Cookies["ht_apkv"].Value;
                var udetails = CreateUser.GetUserDetails(conHT, Request.Cookies["ht_aid"].Value);
                if (udetails != null)
                {
                    ...
                    if (current_pass_key != stored_pass_key)
                    {
                        ...
                        if (Request.Cookies["ht_aid"] != null)
                        {
                            Response.Cookies["ht_aid"].Expires = TimeStamps.UTCTime().AddDays(-10);
                        }
                        if (Request.Cookies["ht_apkv"] != null)
                        {
                            Response.Cookies["ht_apkv"].Expires = TimeStamps.UTCTime().AddDays(-10);
                        }
                        Response.Redirect("Default.aspx");
                    }
                }
    ...
    protected void btnLock_Click(object sender, EventArgs e)
    {
        try
        {
            if (Request.Cookies["ht_aid"] != null)
            {
                string uid = Request.Cookies["ht_aid"].Value;
                Response.Cookies["ht_aid"].Expires = TimeStamps.UTCTime().AddDays(-10);
    ...
    public void BindMenuLink()
    {
        try
        {
            ...
            if (CreateUser.CheckIfAdmin(conHT, Request.Cookies["ht_aid"].Value) == true)
            {
                strDashLink = "dashboard.aspx";
                List<UserAccess> ua = CreateUser.LinkIfAdmin(conHT).OrderBy(x => x.GroupOrder).ToList();
            ...
            {
                List<UserAccess> ua = CreateUser.GetAllUserAccess(conHT, Request.Cookies["ht_aid"].Value).OrderBy(x => x.GroupOrder).ToList();
